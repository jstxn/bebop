...
Add an AGENTS.md file to bebop that helps AI agents (Claude Code, Codex, Cursor, etc.) understand and effectively use bebop's constraint system.

## Goals

- Agents understand that constraints are being injected automatically
- Agents can help users with bebop commands and troubleshooting
- Agents can assist with creating/editing constraint packs

## Distribution

- **Canonical location:** Root of bebop repo (`AGENTS.md`)
- **User projects:** `bebop init` copies to `.bebop/AGENTS.md`
- **Append behavior:** If AGENTS.md already exists, append bebop section rather than overwriting

## Content Structure

### Section 1: What is Bebop (~70 words)
Brief explanation that constraints are injected automatically, link to repo.

### Section 2: Understanding Your Constraints (~100 words)
- Format breakdown: `[CONSTRAINT_ID]`, description, `(pack/category)`
- Context line explanation
- How constraints are selected (paths, frameworks, structure, config)

### Section 3: Useful Commands (~120 words)
- `bebop stats` - usage metrics
- `bebop compile "prompt"` - preview injection
- `bebop packs` - list packs
- Directive syntax: `&use`, `&skip`, `&pack`

### Section 4: Helping Users (~100 words)
- When to suggest directives
- Troubleshooting basics (hook config, context detection, stats)
- Answering constraint questions

### Section 5: Creating/Editing Packs (~150 words)
- Pack file structure (frontmatter, triggers, constraints)
- Creating new packs
- Editing existing packs

## Implementation Tasks

1. Create `AGENTS.md` in bebop repo root
2. Update `bebop init` to copy/append AGENTS.md to `.bebop/AGENTS.md`
3. Update documentation index to reference AGENTS.md
